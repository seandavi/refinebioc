...

{
    ...
    token = rb_get_token()
    operation = rapiclient::get_operations(client)$dataset_update
    if(!is.null(token)) {
        headers = c('API-KEY'=token)
        operation = rapiclient::get_operations(client,.headers=headers)$dataset_update
    }
